...
import org.yhb.dao.service.ICourseTypeDAOService;
import org.yhb.dao.service.IMajorTableDAOService;
...
import org.yhb.util.FileLocation;
import org.yhb.util.FinalDatas;
import org.yhb.util.PageSplit;
import org.yhb.util.PropertiesReadUtil;
...
public class CourseController {
	...
	public String viewCourse(HttpServletRequest request, ModelMap modelMap)
		...
		if (idStr != null && !"".equals(idStr)) {
			...
			ICourseTableDAOService dao = (ICourseTableDAOService) BeanFactory
					.getBean("ICourseTableDAOService");
			CourseTable course = dao.doGetACourseById(id);
			modelMap.put("course", course);
			
			UserTable currentUser = (UserTable) request.getSession().getAttribute("user");
			if(currentUser.getRole()==3){
				ITimeTableDAOService confiDao=(ITimeTableDAOService)BeanFactory.
			...
			// 根据选题查找指导老师信息
			IUserTableDAOService userDao = (IUserTableDAOService) BeanFactory
					.getBean("IUserTableDAOService");
	...
	public String editCourse(HttpServletRequest request, ModelMap map) {
		...
		if (idStr != null && !"".equals(idStr)) {
			...
			// 初始化右边的编辑窗口
			CourseTea course = ((ICourseTableDAOService) BeanFactory
					.getBean("ICourseTableDAOService"))
			...
			if (file != null) {
				...
				for (int i = 2; i < sheet.getRows(); i++) {
					...
					if(title!=null && !"".equals(title)
						...
						try {
							//课题名称存在
							ICourseTableDAOService dao=(ICourseTableDAOService)
								BeanFactory.getBean("ICourseTableDAOService");
    ...
    public String exportCourses(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
    	...
    	if(user.getRole()==1||user.getRole()==2){
			...
			Properties pro=read.read(FileLocation.FINAL_STRING_LOCATION);
	        
	        //创建表格头栏目
	        sheet.mergeCells(0, 0, 8, 0);
	        sheet.addCell(new Label(0,0,pro.getProperty("excelTitleOfTeacher")));
	        log.info(pro.getProperty("excelTitleOfTeacher"));
	        ...
	        sheet.addCell(new Label(3,2,"邮箱"));
	        sheet.addCell(new Label(4,2,"电话"));
	        sheet.addCell(new Label(5,2,"毕业设计（论文）题目"));
	        sheet.addCell(new Label(6,2,"题目来源"));
	        sheet.addCell(new Label(7,2,"题目类型"));
	        sheet.addCell(new Label(8,2,"指导教师"));
	        sheet.addCell(new Label(9,2,"职称"));
	        sheet.addCell(new Label(8,2,"备注"));
	        
	        //课题操作
	        ICourseTableDAOService dao=(ICourseTableDAOService)BeanFactory.getBean("ICourseTableDAOService");
	        ...
	        if(course!=null && course.size()>0){
	    		...
	    		IUserTableDAOService userDao=(IUserTableDAOService)
	    							BeanFactory.getBean("IUserTableDAOService");
	    		IClassTableDAOService classDao=(IClassTableDAOService)
	    							BeanFactory.getBean("IClassTableDAOService");
	    		IMajorTableDAOService majorDao=(IMajorTableDAOService)
	    							BeanFactory.getBean("IMajorTableDAOService");
	    		ICourseSourceDAOService sourceDao=(ICourseSourceDAOService)BeanFactory.getBean("ICourseSourceDAOService");
				ICourseTypeDAOService typeDao=(ICourseTypeDAOService)BeanFactory.getBean("ICourseTypeDAOService");
				IPositionTableDAOService positionDao=(IPositionTableDAOService)BeanFactory.getBean("IPositionTableDAOService");
	    		...
	    		while(ite.hasNext()){
	                ...
	                sheet.addCell(new Label(3, i, courDetTemp.getStuEmail()));
	                sheet.addCell(new Label(4, i, courDetTemp.getStuPhone()));
	                sheet.addCell(new Label(5, i, courDetTemp.getCourseName()));
	                sheet.addCell(new Label(6, i, courDetTemp.getStuCourseSourceChar()));
	                sheet.addCell(new Label(7, i, courDetTemp.getStuCourseTypeChar()));
	                sheet.addCell(new Label(8, i, courDetTemp.getTeaName()));
	                sheet.addCell(new Label(9, i, courDetTemp.getTeaPositionName()));
	                i++;
	    		}
	    		//输出表格尾
