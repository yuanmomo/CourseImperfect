...
import org.yhb.dao.service.ValidationService;
...
public class UserController {
	...
	public String viewListTeachers(HttpServletRequest req,
		...
		if(user.getRole()==1){
			...
			// 查询要列表显示的教师
			ICourseTableDAOService courseDao=(ICourseTableDAOService)
				BeanFactory.getBean("ICourseTableDAOService");
			IUserTableDAOService dao=(IUserTableDAOService)BeanFactory.
	...
	public String editTeacher(HttpServletRequest request, ModelMap map) {
		...
		if (idStr != null && !"".equals(idStr)) {
			...
			// 初始化右边的编辑窗口
			IUserTableDAOService dao=(IUserTableDAOService)BeanFactory.getBean("IUserTableDAOService");
			TeacherPosition teacher=dao.getTeacherPositionById(id);
			if(user.getRole()==1){
				request.setAttribute("module", "tea_edit");
			}
	...
	public String deleteUser(HttpServletRequest request, ModelMap modelMap)
		...
		if (user.getRole() == 1) {// 是管理员
			if (request.getParameter("id") != null
				...
				if(u!=null){
					ICourseTableDAOService courseDao = (ICourseTableDAOService) BeanFactory
...
//		}
			...
			if (file != null) {
				...
				for (int i = 1; i < sheet.getRows(); i++) {
					...
					if(userName!=null&& !"".equals(userName)){
						...
						//初始化该Map
						IPositionTableDAOService dao=(IPositionTableDAOService)BeanFactory.getBean("IPositionTableDAOService");
						...
						try {
							IUserTableDAOService d=(IUserTableDAOService)
	...
	public String editStudent(HttpServletRequest request, ModelMap map) {
		...
		//初始化班级信息
		IClassTableDAOService classDao=(IClassTableDAOService)BeanFactory.getBean("IClassTableDAOService");
		//取得所有的班级
	...
	public String saveStudent(HttpServletRequest request, ModelMap modelMap)
		...
		if (stuNumber == null || "".equals(stuNumber)
			...
			if(!stuNumber.startsWith("3")){
				...
				boolean isLegal=ValidationService.validateUserAvailable(user);
				if(!isLegal){
...
//					u.setPhone(phone);
    ...
    public String exportTeachers(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
    	...
    	if(user.getRole()==1){
	        ...
	        IUserTableDAOService userDao=(IUserTableDAOService)
					BeanFactory.getBean("IUserTableDAOService");
	        IPositionTableDAOService posiDao=(IPositionTableDAOService)BeanFactory.getBean("IPositionTableDAOService");
    ...
    public String exportStudents(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
    	...
    	if(user.getRole()==1){
			...
			ICourseTableDAOService courseDao=(ICourseTableDAOService)BeanFactory.getBean("ICourseTableDAOService");
	        IPositionTableDAOService posintionDao=(IPositionTableDAOService)BeanFactory.getBean("IPositionTableDAOService");
			ICourseSourceDAOService courseSourceDao=(ICourseSourceDAOService)BeanFactory.getBean("ICourseSourceDAOService");
			ICourseTypeDAOService courseTypeDao=(ICourseTypeDAOService)BeanFactory.getBean("ICourseTypeDAOService");
